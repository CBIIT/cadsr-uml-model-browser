...
<html:hidden value=""
	property="<%=UMLBrowserNavigationConstants.METHOD_PARAM%>" />
 ...
 -->
  ...
  <td align="right" width="20%" nowrap>
     ...
     -->
		...
		<logic:equal name="<%=UMLBrowserFormConstants.CLASS_VIEW%>"
			value="true">
			<logic:present
      ...
      No Matches 
		...
		<logic:notEqual name="<%=UMLBrowserFormConstants.CLASS_VIEW%>"
			value="true">
      ...
      No Matches 
		...
		<td align="right" width="20%" nowrap>
			<html:link
				page="/umlbrowser/gotoSearchPreferencesAction.do?method=gotoSearchPreferences"
				target="_parent">
             Search preferences
           </html:link>
		</td>
	...
	<tr>
		<td align="center" colspan="3">
			<html:img page="/i/beigedot.gif" border="0" height="1" width="99%"
				align="top" />
		</td>
	</tr>
</table>

<table align="center" valign="top" width="100%" cellpadding="0"
	cellspacing="1" class="OraBGAccentVeryDark" border="0">

	<tr>
		<td width="10%" class="OraTableColumnHeaderNoBG" nowrap>
			Class Name
		</td>
		<td class="OraTabledata" nowrap>
			<html:text property="<%=UMLBrowserFormConstants.CLASS_NAME%>"
				size="35"
				onkeypress="if (event.keyCode==13){submitForm('classSearch')};" />
		</td>
		<td width="10%" class="OraTableColumnHeaderNoBG" nowrap>
			Attribute Name
		</td>
	...
	<tr>
		<td width="50%" valign="top">
			<table width="100%" cellpadding="0" cellspacing="1"
				class="OraBGAccentVeryDark" border="0"%>
				<tr>
					<td width="10%" class="OraTableColumnHeaderNoBG" nowrap>
						UML Project Name
					</td>
					<td class="OraTabledata" nowrap width="50%">
						<html:select styleClass="FreeDropdown"
							property="<%=UMLBrowserFormConstants.PROJECT_IDSEQ%>"
							onchange="submitForm('resetSubProjPkgOptions')">
							<html:option key="cadsr.umlbrowser.form.blank"
								value="<%=UMLBrowserFormConstants.SEARCH_ALL%>" />

							<logic:iterate id="umlProj"
								name="<%=UMLBrowserFormConstants.ALL_PROJECTS%>"
            ...
            if (umlProj.getId().equals(dynaForm.get(UMLBrowserFormConstants.PROJECT_IDSEQ))) { %>
					...
					<td width="20%" class="OraTableColumnHeaderNoBG" nowrap>
						Version
					</td>
					<td class="OraTabledata" nowrap width="20%">
						<html:text property="<%=UMLBrowserFormConstants.PROJECT_VERSION%>"
							size="10"
							onkeypress="if (event.keyCode==13){submitForm('classSearch')};" />
					</td>
				</tr>
				<tr>
					...
					<td width="70%" class="OraTabledata" nowrap colspan="3">
						<html:select styleClass="FreeDropdown"
							property="<%=UMLBrowserFormConstants.SUB_PROJECT_IDSEQ%>"
							onchange="submitForm('resetPkgOptions')">
							<html:option key="cadsr.umlbrowser.form.blank"
								value="<%=UMLBrowserFormConstants.SEARCH_ALL%>" />
							<html:options
								collection="<%=UMLBrowserFormConstants.SUBPROJECT_OPTIONS%>"
				...
				<tr>
					...
					<td width="70%" class="OraTabledata" nowrap colspan="3">
						<html:select styleClass="FreeDropdown"
							property="<%=UMLBrowserFormConstants.PACKAGE_IDSEQ%>">
							<html:option key="cadsr.umlbrowser.form.blank"
								value="<%=UMLBrowserFormConstants.SEARCH_ALL%>" />
							<html:options
								collection="<%=UMLBrowserFormConstants.PACKAGE_OPTIONS%>"
								property="id" labelProperty="name" />
						</html:select>
		...
		<td align="center" nowrap>
			<table align="center">
				<tr>
					<td nowrap>
						<a
							href="javascript:submitForm('<%=UMLBrowserNavigationConstants.CLASS_SEARCH_METHOD%>')"><img
								src= "<%=request.getContextPath()%>/i/class_search.gif" border=0>
						</a>
					</td>
					<td nowrap>
						<a
							href="javascript:submitForm('<%=UMLBrowserNavigationConstants.ATTRIBUTE_SEARCH_METHOD%>')"><img
								src= "<%=request.getContextPath()%>/i/attribute_search.gif" border=0>
						</a>
					</td>
					<td>
						<a href="javascript:clearForm()"><img src= "<%=request.getContextPath()%>/i/reset.gif" border=0>
						</a>
					</td>
